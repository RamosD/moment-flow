"""Fixtures for reports/media-kit tests."""
...
@pytest.fixture
def owner(make_user):
    return make_user("reports-owner@example.com")


@pytest.fixture
def workspace(seeded, owner):
    return create_workspace(user=owner, name="Reports WS")


@pytest.fixture
def other_owner(make_user):
    return make_user("reports-owner2@example.com")


@pytest.fixture
def other_workspace(seeded, other_owner):
    return create_workspace(user=other_owner, name="Other Reports WS")


@pytest.fixture
def make_artist():
    def _make_artist(workspace, name="Artist", slug="artist"):
        return Artist.objects.create(workspace=workspace, name=name, slug=slug)

    return _make_artist


@pytest.fixture
def make_campaign(make_artist):
    def _make_campaign(workspace, artist=None, name="Campaign", slug="campaign"):
        artist = artist or make_artist(workspace)
